...
class PLC101(PLC):
    ...
    def main_loop(self):
        ...
        # Xhat with attack
        xhat = np.matmul(Aobsv-np.matmul(np.matmul(Gobsv,Cobsv),Aobsv),xhat) + np.matmul(np.matmul(Bobsv-np.matmul(Gobsv,Cobsv),Bobsv),self.prev_inc_i) + np.matmul(Gobsv,self.ya)
