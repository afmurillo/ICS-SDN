...
class TankControl():
	...
	def init(self):
		...
		self.th_uio_on_1 = 0.003*2				   
		#self.th_uio_on_1 = 0.045
		#self.th_uio_on_1 = 0.09		
		self.th_uio_on_2 = 0.05
	...
	def plant_model(self, l, t, q):
		...
		if self.count >=  self.attack_time_begin and self.count <= self.attack_time_end:
			if self.bad_lit_flag == 1:
				...
				self.ya[0,0] = self.abs_attack_value
		...
		if abs(self.ruio1[0]) >= self.th_uio_on_1:
			self.tim_uio_1 = 1
		else:
			self.tim_uio_1 = 0

		if abs(self.ruio2[1]) >= self.th_uio_on_2:
			self.tim_uio_2 = 1
		else:
			self.tim_uio_2 = 0
		...
		while(self.count <= PP_SAMPLES):
			...
			if self.count >=  self.attack_time_begin and self.count <= self.attack_time_end:
				if self.bad_lit_flag == 3:
					self.lit101 = self.lit101 + self.diff_attack_value
